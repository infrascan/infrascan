...
function getPolicyStatementsForRole(role: IamRole): PolicyStatements {
  const inlineStatements = jmespath.search(role, 'inlinePolicies[].PolicyDocument.Document[].Statement');
  const attachedStatements = jmespath.search(role, 'attachedPolicies[].Document.Statement');
  return {
    inlineStatements,
...
function generateEdges(originId: string, allArns: string[], role: IamRole): Edge[] {
  const { 
    inlineStatements, 
    attachedStatements
  } = getPolicyStatementsForRole(role);
  console.log('role statements', { role, inlineStatements, attachedStatements });
  ...
  const effectiveInlineResources: any = inlineStatements?.flatMap(({ Resource }) => {
  ...
  const effectiveAttachedResources: any = attachedStatements?.flatMap(({ Resource }) => {
...
function getEcsRoles(state: EcsTask): string[] {
  return jmespath.search(state, 'TaskDefinition.[executionRoleArn, taskRoleArn]');
}
...
export function generateServiceMap(accountState: AccountState) {
  const allArns: string[] = Object.values(accountState)
    .flatMap((resourceCollection) => {
  ...
  const graphData = Object.values(accountState).flatMap((resourceCollection) => {
    return resourceCollection.map((resource: any) => ({
      ...
      data: {
        ...
        name: resource.Name || resource.Arn || resource?.TaskDefinition?.taskDefinitionArn,
        node_data: resource
      }
    }));
  });
  ...
  const edges = accountState.ecs.flatMap((task) => {
    ...
    if(executionRoleArn) {
      ...
      console.log('evaluating edges for execution role', { role, taskDefArn: task.TaskDefinition?.taskDefinitionArn });
    ...
    if(taskRoleArn) {
      ...
      console.log('evluating edges for task role', { role, taskDefArn: task.TaskDefinition?.taskDefinitionArn });
      if(role && task.TaskDefinition?.taskDefinitionArn) {
  ...
  writeFileSync(`./static/graph-${Date.now()}.json`, JSON.stringify(graphData.concat(edges.concat(lambdaEdges)), undefined, 2), {  });
  return graphData;
}
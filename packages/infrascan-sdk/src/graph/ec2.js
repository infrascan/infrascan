...
async function generateNodesForEc2Networking(account, region) {
	let ec2NetworkingState = [];
	const vpcsState = await evaluateSelector(
		account,
		region,
		'EC2|describeVpcs|[]'
	);
	...
	const availabilityZoneState = await evaluateSelector(
		account,
		region,
	...
	const subnetsState = await evaluateSelector(
		account,
		region,
		'EC2|describeSubnets|[]'
	);

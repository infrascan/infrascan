...
async function performScan(
  ...
  try {
    ...
    do {
      const DescribeLogGroupsCmd = new DescribeLogGroupsCommand({
        nextToken: DescribeLogGroupsPagingToken,
      } as DescribeLogGroupsCommandInput);
      const result: DescribeLogGroupsCommandOutput = await CloudWatchLogs.send(
        DescribeLogGroupsCmd,
      );
      DescribeLogGroupsState.push({
  ...
  const DescribeSubscriptionFiltersParameterResolvers = [
    {
      Key: "logGroupName",
      Selector:
  ...
  const DescribeSubscriptionFiltersParameters =
    (await resolveFunctionCallParameters(
      account,
      region,
      DescribeSubscriptionFiltersParameterResolvers,
  ...
  for (const requestParameters of DescribeSubscriptionFiltersParameters) {
    try {
      ...
      let DescribeSubscriptionFiltersPagingToken: string | undefined =
        undefined;
      do {
        requestParameters["nextToken"] = DescribeSubscriptionFiltersPagingToken;
        const DescribeSubscriptionFiltersCmd =
          new DescribeSubscriptionFiltersCommand(requestParameters);
        const result: DescribeSubscriptionFiltersCommandOutput =
          await CloudWatchLogs.send(DescribeSubscriptionFiltersCmd);
        DescribeSubscriptionFiltersState.push({
          _metadata: { account, region },
  ...
  await onServiceCallComplete(
    ...
    DescribeSubscriptionFiltersState,
  );
}

...
async function performScan(
  ...
  const ListServicesParameterResolvers = [
    { Key: "cluster", Selector: "ECS|ListClusters|[]._result.clusterArns[]" },
    { Key: "maxResults", Value: 100 },
  ];
  const ListServicesParameters = (await resolveFunctionCallParameters(
    account,
    region,
    ListServicesParameterResolvers,
  ...
  for (const requestParameters of ListServicesParameters) {
    try {
      ...
      do {
        ...
        const result: ListServicesCommandOutput = await ECS.send(
          ListServicesCmd,
        );
        ListServicesState.push({
  ...
  const DescribeServicesParameterResolvers = [
  ...
  const DescribeServicesParameters = (await resolveFunctionCallParameters(
    account,
    region,
    DescribeServicesParameterResolvers,
    resolveStateForServiceCall,
  ...
  for (const requestParameters of DescribeServicesParameters) {
    try {
      ...
      do {
        const DescribeServicesCmd = new DescribeServicesCommand(
          requestParameters,
        );
        const result: DescribeServicesCommandOutput = await ECS.send(
          DescribeServicesCmd,
        );
        DescribeServicesState.push({
          _metadata: { account, region },
          _parameters: requestParameters,
          _result: result,
        });

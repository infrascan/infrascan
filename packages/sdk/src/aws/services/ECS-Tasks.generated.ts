...
async function performScan(
  ...
  const ListTasksParameterResolvers = [
    { Key: "cluster", Selector: "ECS|ListClusters|[]._result.clusterArns[]" },
  ];
  const ListTasksParameters = (await resolveFunctionCallParameters(
    account,
    region,
    ListTasksParameterResolvers,
  ...
  const DescribeTasksParameterResolvers = [
    { Key: "cluster", Selector: "ECS|ListTasks|[]._parameters.cluster" },
    { Key: "tasks", Selector: "ECS|ListTasks|[]._result.taskArns" },
  ];
  const DescribeTasksParameters = (await resolveFunctionCallParameters(
    account,
    region,
    DescribeTasksParameterResolvers,
    resolveStateForServiceCall,
  ...
  for (const requestParameters of DescribeTasksParameters) {
    try {
      ...
      do {
        ...
        const result: DescribeTasksCommandOutput = await ECS.send(
          DescribeTasksCmd,
        );
        DescribeTasksState.push({
          _metadata: { account, region },
          _parameters: requestParameters,
          _result: result,
        });
  ...
  const DescribeTaskDefinitionParameterResolvers = [
    {
      Key: "taskDefinition",
  ...
  const DescribeTaskDefinitionParameters = (await resolveFunctionCallParameters(
    account,
    region,
    DescribeTaskDefinitionParameterResolvers,
    resolveStateForServiceCall,
  ...
  for (const requestParameters of DescribeTaskDefinitionParameters) {
    try {
      ...
      do {
        const DescribeTaskDefinitionCmd = new DescribeTaskDefinitionCommand(
          requestParameters,
        );
        const result: DescribeTaskDefinitionCommandOutput = await ECS.send(
          DescribeTaskDefinitionCmd,
        );
        DescribeTaskDefinitionState.push({
          _metadata: { account, region },
          _parameters: requestParameters,
          _result: result,
        });
        const iamRoleSelectors = [

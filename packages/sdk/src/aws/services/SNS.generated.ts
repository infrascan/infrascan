...
async function performScan(
  ...
  const GetTopicAttributesParameterResolvers = [
    { Key: "TopicArn", Selector: "SNS|ListTopics|[]._result[].TopicArn" },
  ];
  const GetTopicAttributesParameters = (await resolveFunctionCallParameters(
    account,
    region,
    GetTopicAttributesParameterResolvers,
  ...
  for (const requestParameters of GetTopicAttributesParameters) {
    try {
      ...
      do {
        const GetTopicAttributesCmd = new GetTopicAttributesCommand(
          requestParameters,
        );
        const result: GetTopicAttributesCommandOutput = await SNS.send(
          GetTopicAttributesCmd,
        );
        const formattedResult = Formatters.SNS.getTopicAttributes(result);
        GetTopicAttributesState.push({
          _metadata: { account, region },
  ...
  await onServiceCallComplete(
    ...
    "SNS",
    "GetTopicAttributes",
    GetTopicAttributesState,
  );

  const ListSubscriptionsByTopicState: GenericState[] = [];
  const ListSubscriptionsByTopicParameterResolvers = [
    { Key: "TopicArn", Selector: "SNS|ListTopics|[]._result[].TopicArn" },
  ];
  const ListSubscriptionsByTopicParameters =
    (await resolveFunctionCallParameters(
      account,
      region,
      ListSubscriptionsByTopicParameterResolvers,
      resolveStateForServiceCall,
  ...
  for (const requestParameters of ListSubscriptionsByTopicParameters) {
    try {
      ...
      do {
        const ListSubscriptionsByTopicCmd = new ListSubscriptionsByTopicCommand(
          requestParameters,
        );
        const result: ListSubscriptionsByTopicCommandOutput = await SNS.send(
          ListSubscriptionsByTopicCmd,
        );
        const formattedResult = Formatters.SNS.listSubscriptionByTopic(result);
        ListSubscriptionsByTopicState.push({
          _metadata: { account, region },
          _parameters: requestParameters,
          _result: formattedResult,
        });
  ...
  await onServiceCallComplete(
    ...
    ListSubscriptionsByTopicState,
  );

  const ListTagsForResourceState: GenericState[] = [];
  const ListTagsForResourceParameterResolvers = [
    { Key: "ResourceArn", Selector: "SNS|ListTopics|[]._result[].TopicArn" },
  ];
  const ListTagsForResourceParameters = (await resolveFunctionCallParameters(
    account,
    region,
    ListTagsForResourceParameterResolvers,
    resolveStateForServiceCall,
  ...
  for (const requestParameters of ListTagsForResourceParameters) {
    try {
      ...
      do {
        const ListTagsForResourceCmd = new ListTagsForResourceCommand(
          requestParameters,
        );
        const result: ListTagsForResourceCommandOutput = await SNS.send(
          ListTagsForResourceCmd,
        );
        ListTagsForResourceState.push({
          _metadata: { account, region },
          _parameters: requestParameters,
          _result: result,
        });

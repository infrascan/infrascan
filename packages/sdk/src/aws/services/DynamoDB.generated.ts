...
async function performScan(
  ...
  try {
    ...
    do {
      ...
      const result: ListTablesCommandOutput = await DynamoDB.send(
        ListTablesCmd,
      );
      const formattedResult = Formatters.DynamoDB.listTables(result);
      ListTablesState.push({
  ...
  const DescribeTableParameterResolvers = [
    { Key: "TableName", Selector: "DynamoDB|ListTables|[]._result[]" },
  ];
  const DescribeTableParameters = (await resolveFunctionCallParameters(
    account,
    region,
    DescribeTableParameterResolvers,
  ...
  for (const requestParameters of DescribeTableParameters) {
    try {
      ...
      do {
        ...
        const result: DescribeTableCommandOutput = await DynamoDB.send(
          DescribeTableCmd,
        );
        const formattedResult = Formatters.DynamoDB.describeTable(result);
        DescribeTableState.push({
          _metadata: { account, region },

...
async function performScan(
  ...
  const GetBucketTaggingParameterResolvers = [
    { Key: "Bucket", Selector: "S3|ListBuckets|[]._result[].Name" },
  ];
  const GetBucketTaggingParameters = (await resolveFunctionCallParameters(
    account,
    region,
    GetBucketTaggingParameterResolvers,
  ...
  for (const requestParameters of GetBucketTaggingParameters) {
    try {
      ...
      do {
        const GetBucketTaggingCmd = new GetBucketTaggingCommand(
          requestParameters,
        );
        const result: GetBucketTaggingCommandOutput = await S3.send(
          GetBucketTaggingCmd,
        );
        GetBucketTaggingState.push({
          _metadata: { account, region },
  ...
  await onServiceCallComplete(
    ...
    "S3",
    "GetBucketTagging",
    GetBucketTaggingState,
  );

  const GetBucketNotificationConfigurationState: GenericState[] = [];
  const GetBucketNotificationConfigurationParameterResolvers = [
    { Key: "Bucket", Selector: "S3|ListBuckets|[]._result[].Name" },
  ];
  const GetBucketNotificationConfigurationParameters =
    (await resolveFunctionCallParameters(
      account,
      region,
      GetBucketNotificationConfigurationParameterResolvers,
      resolveStateForServiceCall,
  ...
  for (const requestParameters of GetBucketNotificationConfigurationParameters) {
    try {
      ...
      let GetBucketNotificationConfigurationPagingToken: string | undefined =
        undefined;
      do {
        const GetBucketNotificationConfigurationCmd =
          new GetBucketNotificationConfigurationCommand(requestParameters);
        const result: GetBucketNotificationConfigurationCommandOutput =
          await S3.send(GetBucketNotificationConfigurationCmd);
        GetBucketNotificationConfigurationState.push({
          _metadata: { account, region },
          _parameters: requestParameters,
          _result: result,
        });
  ...
  await onServiceCallComplete(
    ...
    GetBucketNotificationConfigurationState,
  );

  const GetBucketWebsiteState: GenericState[] = [];
  const GetBucketWebsiteParameterResolvers = [
    { Key: "Bucket", Selector: "S3|ListBuckets|[]._result[].Name" },
  ];
  const GetBucketWebsiteParameters = (await resolveFunctionCallParameters(
    account,
    region,
    GetBucketWebsiteParameterResolvers,
    resolveStateForServiceCall,
  ...
  for (const requestParameters of GetBucketWebsiteParameters) {
    try {
      ...
      do {
        const GetBucketWebsiteCmd = new GetBucketWebsiteCommand(
          requestParameters,
        );
        const result: GetBucketWebsiteCommandOutput = await S3.send(
          GetBucketWebsiteCmd,
        );
        GetBucketWebsiteState.push({
          _metadata: { account, region },
          _parameters: requestParameters,
          _result: result,
        });
  ...
  await onServiceCallComplete(
    ...
    "S3",
    "GetBucketWebsite",
    GetBucketWebsiteState,
  );

  const GetBucketAclState: GenericState[] = [];
  const GetBucketAclParameterResolvers = [
    { Key: "Bucket", Selector: "S3|ListBuckets|[]._result[].Name" },
  ];
  const GetBucketAclParameters = (await resolveFunctionCallParameters(
    account,
    region,
    GetBucketAclParameterResolvers,
    resolveStateForServiceCall,
  ...
  for (const requestParameters of GetBucketAclParameters) {
    try {
      ...
      do {
        ...
        const result: GetBucketAclCommandOutput = await S3.send(
          GetBucketAclCmd,
        );
        GetBucketAclState.push({
          _metadata: { account, region },
          _parameters: requestParameters,
          _result: result,
        });

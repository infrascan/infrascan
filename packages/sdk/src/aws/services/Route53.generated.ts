...
async function performScan(
  ...
  try {
    ...
    do {
      const ListHostedZonesByNameCmd = new ListHostedZonesByNameCommand(
        {} as ListHostedZonesByNameCommandInput,
      );
      const result: ListHostedZonesByNameCommandOutput = await Route53.send(
        ListHostedZonesByNameCmd,
      );
      const formattedResult = Formatters.Route53.listHostedZonesByName(result);
      ListHostedZonesByNameState.push({
  ...
  await onServiceCallComplete(
    ...
    ListHostedZonesByNameState,
  );

  const ListResourceRecordSetsState: GenericState[] = [];
  const ListResourceRecordSetsParameterResolvers = [
    {
      Key: "HostedZoneId",
      Selector: "Route53|ListHostedZonesByName|[]._result[].Id",
    },
  ];
  const ListResourceRecordSetsParameters = (await resolveFunctionCallParameters(
    account,
    region,
    ListResourceRecordSetsParameterResolvers,
  ...
  for (const requestParameters of ListResourceRecordSetsParameters) {
    try {
      ...
      do {
        const ListResourceRecordSetsCmd = new ListResourceRecordSetsCommand(
          requestParameters,
        );
        const result: ListResourceRecordSetsCommandOutput = await Route53.send(
          ListResourceRecordSetsCmd,
        );
        const formattedResult =
          Formatters.Route53.listResourceRecordSets(result);
        ListResourceRecordSetsState.push({
          _metadata: { account, region },

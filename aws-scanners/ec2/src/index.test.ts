...
t.test(
  ...
  async () => {
    ...
    t.ok(
      nodes.find(
        (node) =>
          node.id === `${testVpcId}-${testAZ1}` && node.parent === testVpcId,
      ),
    );
    t.ok(
      nodes.find(
        (node) =>
          node.id === `${testVpcId}-${testAZ2}` && node.parent === testVpcId,
      ),
    );
    ...
    t.ok(
      nodes.find(
        (node) =>
          node.id === testSubnetId1 &&
          node.parent === `${testVpcId}-${testAZ1}`,
      ),
    );
    t.ok(
      nodes.find(
        (node) =>
          node.id === testSubnetId2 &&
          node.parent === `${testVpcId}-${testAZ2}`,
      ),
    );
  },
);
